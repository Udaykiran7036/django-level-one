...
#         return JsonResponse({"status":"success","stars":stars}, status=200)
...
def movies(request):
   ...
   #PUT
       ...
       ref_id=data.get("id")
       print("reference ID:",ref_id) #check the id coming from thr client
       existing_movie=Movie_details.objects.get(id=ref_id)
       print("existing_movie:",existing_movie) #check the existing movie object fetched from db 
...
#POST
    ...
    #    data=json.loads(request.body) #whenever we send data in json formst we have to use this
       data=request.POST #whenever we send data in form format we have to use this
